...
def register():
    ...
    if form.validate_on_submit():
        password_hash = bcrypt_.generate_password_hash(form.password.data).decode('utf-8')
        ...
        db_session.add(user)
        db_session.commit()
        flash(f'{form.username.data.capitalize()} your account has been successfully created ')
